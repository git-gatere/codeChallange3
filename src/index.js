document.addEventListener('DOMContentLoaded', () => {
    //Runs after html file is loaded

    const movieList = document.getElementById('films');
    // stores the array with the id 'films' in the variable 'movieList'.

    let movieData = [];
    // Declares an array

    function fetchMoviesFromDB() {
        // This function fetches movie data.
        fetch('db.json')
            .then(response => {
            ...
            .then(data => {
                // store movie data in the 'movieData' array.
                movieData = data.films;
                // Hoist the 'displayMovies' function to show the movies on the webpage.
                displayMovies();
            })
            .catch(error => {
